/* Recicla Interativo — script.js
   - Tema escuro UI (styles.css)
   - Storage key: 'ri_state'
   - Default map center: Bertolínia, PI
   - Uses Leaflet + Nominatim + QRCode
*/
...
/* ---------- Classifier (manual + file heuristic) ---------- */
...
/* ---------- Schools (Nominatim search around city) ---------- */
async function fetchNearbySchools(){
  // search for "school" around Bertolínia bounding box or city query
  ...
  try{
    ...
    alert('Escolas adicionadas ao mapa (fictícias/OSM). Verifique os marcadores.');
...
/* ---------- Init helper calls ---------- */
function renderSupportList(){ /* already called on load */ }
